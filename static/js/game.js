...
gameCaption.textContent= `Connect the world with ${moduleValue} 🌍`

// Construct the URL with the dynamic parameter
const dynamicUrl = 'game.html?' + dynamicParamKey + '=' + moduleValue;

// variable for quiz count
let quizCount = 5;

// // words to use in the quiz
// if (moduleValue === "adjectives") {
//   // focusing on 4s
//   var words = categories[moduleValue].filter((word) => word.length === 6);
//   var quizCount = words.length;
// }
//  else {
//   // leaving out bingos and eights since they cant fit on screen
//   var words = categories[moduleValue].filter((word) => word.length < 7);  
// }
...
function createDynamicUrl()
{
  ...
  // Construct the URL with the dynamic parameter
  let dynamicUrl = currentUrl + urlSeparator + dynamicParamKey + '=' + moduleValue;
